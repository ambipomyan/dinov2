...
class _Split(Enum):
    ...
    def get_image_relpath(self, actual_index: int, class_id: Optional[str] = None) -> str:
        ...
        #basename = f"{class_id}_{actual_index}"
        basename = f"{class_id}-{actual_index}"
    ...
    def parse_image_relpath(self, image_relpath: str) -> Tuple[str, int]:
        ...
        actual_index = int(basename.split("-")[1]) # T4-0961-....
        return class_id, actual_index

class EVALDataset(ExtendedVisionDataset):
...
#################
        ...
        for file_name in os.listdir(curr_root):
            ...
            with open(curr_full_file_name, 'rb') as f:
                df_files = pd.read_csv(f, usecols=['imgs'])
                files = df_files.values.tolist()
                for f in files:
                    img_hwc  = imread(f)
### hwc -> chw
                    channels = img_hwc.shape[2]
    ...
    @property
    def split(self) -> "EVALDataset.Split":
        return self._split
    ...
    def _dump_entries(self) -> None:
        split = self.split
        if split == EVALDataset.Split.TEST:
            dataset = None
            ...
            sys.path.append('/home2/s224636/Documents/Spatial_Biology_Project/examples/evalution')
            from eval_dataset import MyEVALImageFolder

            dataset_root = os.path.join(self.root, split.get_dirname())
            #dataset = ImageFolder(dataset_root)
            dataset = MyEVALImageFolder(dataset_root)
